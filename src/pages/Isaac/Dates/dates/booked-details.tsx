  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex gap-4 mb-6">
          <div className="flex-1 bg-[#F3E8FF] rounded-lg p-4">
            ...
            <div className="font-semibold text-gray-900">
              {new Date(dateDetails.scheduled_date).toLocaleDateString()}
            </div>
          </div>
          <div className="flex-1 bg-[#F3E8FF] rounded-lg p-4">
            <div className="text-sm font-medium text-[#5E17EB]">Time</div>
            <div className="font-semibold text-gray-900">{dateDetails.scheduled_time}</div>
          </div>
          <div className="flex-1 bg-[#F3E8FF] rounded-lg p-4">
            <div className="text-sm font-medium text-[#5E17EB]">Status</div>
            <div className="font-semibold text-gray-900">{dateDetails.status}</div>
          </div>
        </div>

        <div className="bg-white rounded-lg p-4 mb-6 flex items-start gap-4">
          <img
            src={dateDetails.place_reservation.place.placePhotos?.[0]?.image_url || "/placeholder.svg"}
            alt={dateDetails.place_reservation.place.name}
            className="w-16 h-16 rounded-lg object-cover"
          />
          <div>
            <h2 className="font-bold text-lg text-gray-900">{dateDetails.place_reservation.place.name}</h2>
            <p className="text-gray-600 text-sm">
              {dateDetails.place_reservation.place.location.city}, {dateDetails.place_reservation.place.location.state}
            </p>
            <div className="flex items-center gap-1 mt-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`w-4 h-4 ${
                    i < dateDetails.place_reservation.place.rating
        ...
        {[dateDetails.booked_by, dateDetails.matched_user].map((participant, index) => (
              ...
              <div className="grid gap-4">
                {index === 0 && (
                  <div>
                    ...
                    <div className="text-gray-600">{dateDetails.pick_up_location}</div>
                  </div>
                )}

                {dateDetails.booked_ride && (
                  <div>
                    <div className="font-semibold text-gray-900 mb-1">Ride</div>
                    <div className="text-gray-600">
                      {dateDetails.booked_ride.car_ride.brand.name} {dateDetails.booked_ride.car_ride.category.name}
                    </div>
                  </div>
                )}

                {dateDetails.booked_security && (
                  <div>
                    <div className="font-semibold text-gray-900 mb-1">Security</div>
                    <div className="flex gap-2 items-center">
                      <span className="text-gray-600">
                        {dateDetails.booked_security.number_of_escorts} escorts from{" "}
                        {dateDetails.booked_security.security.organization_name}
                      </span>
                      <span className="bg-[#F3E8FF] text-[#5E17EB] px-3 py-1 rounded-full text-sm font-medium">
                        Preference: {dateDetails.booked_security.gender_preference}
                      </span>
                    </div>
                  </div>
                )}

                {dateDetails.security_question && index === 0 && (
                  <div>
                    <div className="font-semibold text-gray-900 mb-1">Security Question</div>
                    <div className="text-gray-600">{dateDetails.security_question}</div>
                  </div>
                )}
              </div>
            </div>
